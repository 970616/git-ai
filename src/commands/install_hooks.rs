...
fn install_git_prepush_hook(binary_path: &Path, dry_run: bool) {
    ...
    {
        use std::os::unix::fs::PermissionsExt;
        if let Ok(meta) = fs::metadata(&pre_push_path) {
            let mut perms = meta.permissions();
            perms.set_mode(0o755);
...
# git-ai pre-push: report authorship notes to server
# Installed by `git ai install-hooks` — do not edit manually.
# git-ai pre-push
...
def git(*args):
    try:
        out = subprocess.check_output(("git",) + args, stderr=subprocess.PIPE)
        return out.decode("utf-8", "replace")
    except Exception:
        return ""

def git_config(key, default=""):
    val = git("config", "--get", key).strip()
    return val if val else default

def is_zero(sha):
    return len(set(sha)) == 1 and sha[0] == "0"

def extract_change_id(commit):
    body = git("log", "-1", "--format=%B", commit)
    for line in body.splitlines():
        m = re.match(r"^\s*Change-Id:\s*(I[0-9a-fA-F]+)\s*$", line)
        if m:
            return m.group(1)
    return ""

def target_branch(remote_ref):
    ref = remote_ref
    if "%" in ref:
        ref = ref.split("%", 1)[0]
    for prefix in ("refs/for/", "refs/heads/", "refs/drafts/"):
        if ref.startswith(prefix):
            return ref[len(prefix):]
    return ref

def commits_to_push(local_sha, remote_sha, remote_name):
    if remote_sha and not is_zero(remote_sha):
        rng = git("rev-list", "%s..%s" % (remote_sha, local_sha))
    else:
...
def main():
    ...
    for raw in sys.stdin:
        ...
        if is_zero(local_sha):
            continue
        branch = target_branch(remote_ref)

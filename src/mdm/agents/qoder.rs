...
use crate::mdm::utils::{
    binary_exists, qoder_config_dir, generate_diff, is_git_ai_checkpoint_command,
...
impl QoderInstaller {
    fn settings_path() -> PathBuf {
        qoder_config_dir().join("settings.json")
    }
    ...
    fn hook_status(settings: &Value) -> (bool, bool) {
        ...
        for block in blocks {
            let is_catch_all = block
                .get("matcher")
                .and_then(|m| m.as_str())
                .map(|m| m == QODER_CATCH_ALL_MATCHER)
                .unwrap_or(false);
    ...
    fn install_hooks_at(
        ...
        // Qoder does not support a top-level "env" block; inject
        // GIT_AI_DAEMON_HOME into each hook entry's own env instead, so the
        // checkpoint subprocess connects to the same daemon as the binary.
        let daemon_home = std::env::var("GIT_AI_DAEMON_HOME")
            .ok()
            .filter(|v| !v.is_empty());
        ...
        for (hook_type, desired_cmd) in &[
            ...
            let desired_entry = |cmd: &str| -> Value {
                let mut entry = json!({
                    "type": "command",
                    "command": cmd,
                });
                if let Some(home) = &daemon_home {
                    entry["env"] = json!({ "GIT_AI_DAEMON_HOME": home });
                }
                entry
            };
            ...
            for (idx, hook) in hooks_array.iter().enumerate() {
                ...
                {
                    found_idx = Some(idx);
                    // 命令或 env 有差异都算需要更新（daemon 目录变化时也要刷新）。
                    if hook != &desired_entry(cmd) {
                        needs_update = true;
                    }
                }
            }
            ...
            match found_idx {
                Some(idx) => {
                    if needs_update {
                        hooks_array[idx] = desired_entry(desired_cmd);
                    }
                ...
                None => {
                    hooks_array.push(desired_entry(desired_cmd));
                }
            }
...
impl HookInstaller for QoderInstaller {
    ...
    fn check_hooks(&self, _params: &HookInstallerParams) -> Result<HookCheckResult, GitAiError> {
        ...
        let has_dotfiles = qoder_config_dir().exists();
...
mod tests {
    ...
    fn expected_cmd() -> String {
        format!("{} {}", binary_path().display(), QODER_PRE_TOOL_CMD)
    }
    ...
    /// Removes GIT_AI_DAEMON_HOME for the duration of a test so hand-written
    /// fixtures (which carry no per-entry env) match installed output.
    /// Requires `#[serial_test::serial]` on the test since env is process-global.
    struct DaemonHomeGuard;

    impl DaemonHomeGuard {
        fn clear() -> Self {
            unsafe {
                std::env::remove_var("GIT_AI_DAEMON_HOME");
            }
            DaemonHomeGuard
        }
    }

    impl Drop for DaemonHomeGuard {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("GIT_AI_DAEMON_HOME");
            }
    ...
    fn s1_fresh_install_creates_catch_all_block() {
        ...
        let diff = QoderInstaller::install_hooks_at(&path, &params(), false).unwrap();
    ...
    fn s2_idempotent_already_on_catch_all() {
        let _guard = DaemonHomeGuard::clear();
        let (_td, path) = setup_test_env();
    ...
    fn s3_migration_old_matcher_no_user_hooks() {
        ...
        QoderInstaller::install_hooks_at(&path, &params(), false).unwrap();
    ...
    fn s10_stale_command_upgraded_in_catch_all() {
        ...
        fs::write(
            ...
            serde_json::to_string_pretty(&json!({
                "hooks": {
                    "PreToolUse": [{"matcher": "*", "hooks": [{"type":"command","command": "/old/path/git-ai checkpoint qoder --hook-input stdin"}]}],
                    "PostToolUse": [{"matcher": "*", "hooks": [{"type":"command","command": "/old/path/git-ai checkpoint qoder --hook-input stdin"}]}]
                }
            }))
            .unwrap(),
        )
    ...
    fn u1_uninstall_from_catch_all() {
        ...
        let diff = QoderInstaller::uninstall_hooks_at(&path, false).unwrap();
        assert!(diff.is_some());
    ...
    fn u2_uninstall_from_old_matcher_preserves_user_hook() {
        ...
        QoderInstaller::uninstall_hooks_at(&path, false).unwrap();
    ...
    fn c1_no_hooks_returns_not_installed() {
        let settings = json!({});
        let (installed, up_to_date) = QoderInstaller::hook_status(&settings);
        assert!(!installed);
        assert!(!up_to_date);
    }
    ...
    #[test]
    fn test_qoder_hook_commands_no_windows_extended_path_prefix() {
        ...
        assert!(
            pre_tool_cmd.contains("checkpoint qoder"),
            "command should still contain checkpoint args"
        );
    }

    #[test]
    fn test_qoder_hook_commands_use_forward_slash_path_on_windows() {
        ...
        assert_eq!(
            pre_tool_cmd,
            "C:/Users/Administrator/.git-ai/bin/git-ai.exe checkpoint qoder --hook-input stdin",
    ...
    #[test]
    fn test_qoder_hook_commands_preserve_unix_path() {
        let binary_path = PathBuf::from("/usr/local/bin/git-ai");
        let binary_path_str = normalize_windows_path_for_shell(&binary_path);
        let pre_tool_cmd = format!("{} {}", binary_path_str, QODER_PRE_TOOL_CMD);

        assert_eq!(
            pre_tool_cmd, "/usr/local/bin/git-ai checkpoint qoder --hook-input stdin",
    ...
    fn test_install_hooks_creates_missing_parent_dir() {
        ...
        let result =
            QoderInstaller::install_hooks_at(&settings_path, &params(), false).unwrap();

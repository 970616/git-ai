...
use crate::authorship::ignore::{
    build_ignore_matcher, should_exclude_checkpoint_path, should_ignore_file_with_matcher,
};
...
pub(crate) fn stats_for_commit_stats_from_hunks_with_merge_flag(
    ...
    for hunk in hunks {
        ...
        // 全仓统计，仅排除黑名单（默认 Top18 + GITAI_CHECKPOINT_EXCLUDE）：命中不统计。
        if should_exclude_checkpoint_path(&hunk.file_path) {
            continue;
        }

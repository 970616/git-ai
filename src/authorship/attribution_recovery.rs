...
pub(crate) fn recover_attribution(
    ...
    // Heuristic attribution recovery is disabled by default so that only edits
    // explicitly confirmed by a checkpoint (from a supported AI tool) are marked
    // as AI-authored. Instead of leaving every other committed line
    // "unattributed" (which renders as human in blame but is NOT counted in the
    // commit note's human stats), we positively attribute those remaining lines
    // to the human committer as KnownHuman. This makes manually written code
    // (e.g. from an unsupported IDE) count as human in the note/stats, and
    // prevents it from being probabilistically inferred as AI via the mtime
    // time-window, adjacent-edge, session-event, or commit-metadata solvers
    // below.
    //
    // Set GIT_AI_ENABLE_ATTRIBUTION_RECOVERY=1 to restore the original
    // heuristic recovery behavior.
    if std::env::var("GIT_AI_ENABLE_ATTRIBUTION_RECOVERY").as_deref() != Ok("1") {
        attribute_unknown_lines_to_human(authorship_log, committed_hunks, human_author);
        return Ok(());
    }
...
/// Default-policy recovery: attribute every committed line that is not already
/// covered by an AI or known-human attestation to the human committer, using a
/// KnownHuman ("h_") author id. This positively counts un-checkpointed manual
/// edits as human in the commit note/stats (instead of leaving them "unknown"),
/// while never inferring AI authorship.
fn attribute_unknown_lines_to_human(
    authorship_log: &mut AuthorshipLog,
    committed_hunks: &HashMap<String, Vec<LineRange>>,
    human_author: &str,
) {
    ...
    // Register the human author so blame/show can resolve the h_ hash to a name.
    authorship_log
        .metadata
        .humans
    ...
    for (file_path, unknown_lines) in unknown_by_file {
        add_attestation(authorship_log, &file_path, &human_id, &unknown_lines);
    }
}
